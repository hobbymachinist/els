...
static void els_taper_ext_r_display_diagram(void) {
  ...
  tft_filled_triangle(&tft,  156, 220, 254, 240, 254, 220, TFT_COLOR_ORANGE);
  tft_filled_triangle(&tft,  156, 300, 254, 280, 254, 300, TFT_COLOR_ORANGE);
...
static void els_taper_ext_r_display_header(void) {
  tft_rgb_t color = (els_taper_ext_r.locked ? TFT_COLOR_RED : TFT_COLOR_CERULEAN);

  tft_filled_rectangle(&tft, 0, 0, 480, 50, color);
  tft_font_write_bg(&tft, 8, 0, "TAPER EXTERNAL - R", &noto_sans_mono_bold_26, TFT_COLOR_WHITE, color);
  els_taper_ext_r_display_encoder_pips();
}
...
static void els_taper_ext_r_display_encoder_pips(void) {
  ...
  for (size_t n = 0, spacing = 0; n < 3; n++, spacing++)
    tft_filled_rectangle(&tft,
      ...
      15 + n * 10, 10,
      (n < pips ? TFT_COLOR_WHITE : els_taper_ext_r.locked ? TFT_COLOR_LITEGRAY : TFT_COLOR_BGCOLOR1));
}
  ...
  switch (els_taper_ext_r.op_state) {
    ...
    case ELS_TAPER_EXT_OP_PLAN:
      ...
      els_stepper_move_xz(
        ...
        (els_taper_ext_r.finish_pass_count > 0 ? els_taper_ext_r.feed_mm_s : els_taper_ext_r.feed_mm_s)
      );
      break;
