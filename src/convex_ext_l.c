...
static void els_convex_ext_l_display_diagram(void) {
  ...
  // diagram
  // ----------------------------------------------------------------------------------------------
  tft_filled_rectangle(&tft, 100, 220, 160, 80, TFT_COLOR_WHITE);
  tft_filled_rectangle(&tft, 160, 220, 40, 80, TFT_COLOR_ORANGE);
  tft_filled_circle(&tft, 195, 260, 40, TFT_COLOR_WHITE);
  tft_filled_rectangle(&tft, 140, 220, 20, 22, TFT_COLOR_BLACK);
  tft_filled_rectangle(&tft, 140, 278, 20, 22, TFT_COLOR_BLACK);

  tft_font_write_bg(&tft, 156, 176, "A", &noto_sans_mono_bold_arrows_24, TFT_COLOR_WHITE, TFT_COLOR_BLACK);
  tft_font_write_bg(&tft, 176, 192, "L", &inconsolata_lgc_bold_14, TFT_COLOR_WHITE, TFT_COLOR_BLACK);
  tft_font_write_bg(&tft, 184, 176, "C", &noto_sans_mono_bold_arrows_24, TFT_COLOR_WHITE, TFT_COLOR_BLACK);

  tft_font_write_bg(&tft, 186, 238, "B", &noto_sans_mono_bold_arrows_24, TFT_COLOR_BLUE, TFT_COLOR_WHITE);
  tft_font_write_bg(&tft, 202, 262, "R", &inconsolata_lgc_bold_14, TFT_COLOR_BLUE, TFT_COLOR_WHITE);
  tft_font_write_bg(&tft, 186, 262, "D", &noto_sans_mono_bold_arrows_24, TFT_COLOR_BLUE, TFT_COLOR_WHITE);

  tft_font_write_bg(&tft, 140, 184, "D", &noto_sans_mono_bold_arrows_24, TFT_COLOR_WHITE, TFT_COLOR_BLACK);
  tft_font_write_bg(&tft, 144, 218, "D", &inconsolata_lgc_bold_14, TFT_COLOR_WHITE, TFT_COLOR_BLACK);
  tft_font_write_bg(&tft, 140, 226, "B", &noto_sans_mono_bold_arrows_24, TFT_COLOR_BLACK, TFT_COLOR_WHITE);

  // origin
  tft_font_write_bg(&tft, 150, 265, ".", &noto_sans_mono_bold_26, TFT_COLOR_YELLOW, TFT_COLOR_BLACK);
...
static void els_convex_ext_l_display_header(void) {
  tft_rgb_t color = (els_convex_ext_l.locked ? TFT_COLOR_RED : TFT_COLOR_CERULEAN);
  tft_filled_rectangle(&tft, 0, 0, 480, 50, color);
  tft_font_write_bg(&tft, 8, 0, "CONVEX EXTERNAL - L", &noto_sans_mono_bold_26, TFT_COLOR_WHITE, color);
  els_convex_ext_l_display_encoder_pips();
}
...
static void els_convex_ext_l_display_encoder_pips(void) {
  ...
  for (size_t n = 0, spacing = 0; n < 3; n++, spacing++)
    tft_filled_rectangle(&tft,
      ...
      15 + n * 10, 10,
      (n < pips ? TFT_COLOR_WHITE : els_convex_ext_l.locked ? TFT_COLOR_LITEGRAY : TFT_COLOR_BGCOLOR1));
}
