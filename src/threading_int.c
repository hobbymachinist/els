...
// Internal functions
//==============================================================================
static void els_threading_int_set_zdir(void);
    ...
    if (els_threading_int.state == ELS_THREADING_ACTIVE && els_threading_int.op_state == ELS_THREADING_OP_THREAD) {
      if (reset_pulse_pending) {
        ...
        if (els_config->z_closed_loop) {
          ...
          if ((els_stepper->zdir * dro_zpos) >= els_threading_int.length)
            els_threading_int.op_state = ELS_THREADING_OP_THREADL;
          els_stepper->zpos = dro_zpos;
        }
        else {
          if ((els_stepper->zdir * els_stepper->zpos) >= els_threading_int.length)
            els_threading_int.op_state = ELS_THREADING_OP_THREADL;
        }
      }
    }
...
static void els_threading_int_display_setting(void) {
  ...
  if (els_threading_int.pitch_mode == ELS_THREADING_PITCH_MODE_TABLE) {
    tft_font_write_bg(&tft, x, y, pitch_table_label[els_threading_int.pitch_table_index],
      ...
      (els_threading_int.state == ELS_THREADING_SET_PITCH ? TFT_COLOR_YELLOW : TFT_COLOR_WHITE),
      TFT_COLOR_BLACK);
  }
}
...
static void els_threading_int_display_diagram(void) {
  ...
  // diagram
  // ----------------------------------------------------------------------------------------------
  tft_image_draw(&tft, 100, 235, &internal_thread, TFT_COLOR_WHITE);

  tft_font_write_bg(&tft, 136, 183, "A", &noto_sans_mono_bold_arrows_24, TFT_COLOR_WHITE, TFT_COLOR_BLACK);
  tft_font_write_bg(&tft, 190, 199, "L", &inconsolata_lgc_bold_14, TFT_COLOR_WHITE, TFT_COLOR_BLACK);
  tft_font_write_bg(&tft, 240, 183, "C", &noto_sans_mono_bold_arrows_24, TFT_COLOR_WHITE, TFT_COLOR_BLACK);

  tft_font_write_bg(&tft, 260, 210, "D", &noto_sans_mono_bold_arrows_24, TFT_COLOR_WHITE, TFT_COLOR_BLACK);
  tft_font_write_bg(&tft, 280, 237, "H", &inconsolata_lgc_bold_14, TFT_COLOR_WHITE, TFT_COLOR_BLACK);
  tft_font_write_bg(&tft, 260, 237, "B", &noto_sans_mono_bold_arrows_24, TFT_COLOR_WHITE, TFT_COLOR_BLACK);

  // origin
  tft_font_write_bg(&tft, 245, 260, ".", &noto_sans_mono_bold_26, TFT_COLOR_YELLOW, TFT_COLOR_BLACK);
...
static void els_threading_int_display_header(void) {
  if (els_threading_int.pitch_reverse) {
    tft_rgb_t color = (els_threading_int.locked ? TFT_COLOR_RED : TFT_COLOR_GREEN);
    tft_filled_rectangle(&tft, 0, 0, 480, 50, color);
    tft_font_write_bg(&tft, 8, 0, "THREAD INTERNAL - L", &noto_sans_mono_bold_26, TFT_COLOR_BLACK, color);
  }
  else {
    tft_rgb_t color = (els_threading_int.locked ? TFT_COLOR_RED : TFT_COLOR_FOREST);
    tft_filled_rectangle(&tft, 0, 0, 480, 50, color);
    tft_font_write_bg(&tft, 8, 0, "THREAD INTERNAL - R", &noto_sans_mono_bold_26, TFT_COLOR_WHITE, color);
  }
...
static void els_threading_int_display_encoder_pips(void) {
  ...
  for (size_t n = 0, spacing = 0; n < 3; n++, spacing++)
    tft_filled_rectangle(&tft,
      ...
      15 + n * 10, 10,
      (n < pips ? TFT_COLOR_WHITE : els_threading_int.locked ? TFT_COLOR_LITEGRAY : TFT_COLOR_BGCOLOR2));
}
...
static void els_threading_int_run(void) {
  if (els_threading_int.state == ELS_THREADING_PAUSED && !els_stepper->xbusy && !els_stepper->zbusy) {
    els_threading_int_set_zdir();
