...
static void els_facing_display_diagram(void) {
  ...
  tft_filled_rectangle(&tft, 235, 220,  21, 80, TFT_COLOR_ORANGE);
  tft_filled_rectangle(&tft, 80,  258, 200,  4, TFT_COLOR_LITEGRAY);

  tft_font_write_bg(&tft, 235, 178, "A", &noto_sans_mono_bold_arrows_24, TFT_COLOR_WHITE, TFT_COLOR_BLACK);
  tft_font_write_bg(&tft, 220, 194, "L", &inconsolata_lgc_bold_14, TFT_COLOR_WHITE, TFT_COLOR_BLACK);
  ...
  tft_font_write_bg(&tft, 265, 226, "D", &inconsolata_lgc_bold_14, TFT_COLOR_WHITE, TFT_COLOR_BLACK);
  tft_font_write_bg(&tft, 260, 250, "B", &noto_sans_mono_bold_arrows_24, TFT_COLOR_WHITE, TFT_COLOR_BLACK);

  // origin
  tft_font_write(&tft, 250, 265, ".", &noto_sans_mono_bold_26, TFT_COLOR_YELLOW);
...
static void els_facing_display_header(void) {
  tft_rgb_t color = (els_facing.locked ? TFT_COLOR_RED : TFT_COLOR_DIANNE);

  tft_filled_rectangle(&tft, 0, 0, 480, 50, color);
  tft_font_write_bg(&tft, 8, 0, "FACING", &noto_sans_mono_bold_26, TFT_COLOR_WHITE, color);
  els_facing_display_encoder_pips();
}
...
static void els_facing_display_encoder_pips(void) {
  ...
  for (size_t n = 0, spacing = 0; n < 3; n++, spacing++)
    tft_filled_rectangle(&tft,
      ...
      15 + n * 10, 10,
      (n < pips ? TFT_COLOR_WHITE : els_facing.locked ? TFT_COLOR_LITEGRAY : TFT_COLOR_BGCOLOR1));
}
