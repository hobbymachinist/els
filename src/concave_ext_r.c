...
static void els_concave_ext_r_display_diagram(void) {
  ...
  // diagram
  // ----------------------------------------------------------------------------------------------
  tft_filled_rectangle(&tft, 100, 220, 150, 80, TFT_COLOR_WHITE);
  tft_filled_arc(&tft, 249, 305, 40, 40, 270, 30, 40, TFT_COLOR_ORANGE);
  tft_filled_arc(&tft, 249, 215, 40, 40, 180, 30, 40, TFT_COLOR_ORANGE);
  tft_line(&tft, 210, 300, 255, 300, TFT_COLOR_CERULEAN);
  tft_line(&tft, 210, 220, 250, 220, TFT_COLOR_CERULEAN);

  tft_font_write_bg(&tft, 206, 172, "A", &noto_sans_mono_bold_arrows_24, TFT_COLOR_WHITE, TFT_COLOR_BLACK);
  tft_font_write_bg(&tft, 226, 189, "L", &inconsolata_lgc_bold_14, TFT_COLOR_WHITE, TFT_COLOR_BLACK);
  tft_font_write_bg(&tft, 234, 172, "C", &noto_sans_mono_bold_arrows_24, TFT_COLOR_WHITE, TFT_COLOR_BLACK);

  tft_font_write_bg(&tft, 264, 255, "B", &noto_sans_mono_bold_arrows_24, TFT_COLOR_WHITE, TFT_COLOR_BLACK);
  tft_font_write_bg(&tft, 278, 282, "R", &inconsolata_lgc_bold_14, TFT_COLOR_WHITE, TFT_COLOR_BLACK);
  tft_font_write_bg(&tft, 264, 277, "D", &noto_sans_mono_bold_arrows_24, TFT_COLOR_WHITE, TFT_COLOR_BLACK);

  tft_font_write_bg(&tft, 264, 184, "D", &noto_sans_mono_bold_arrows_24, TFT_COLOR_WHITE, TFT_COLOR_BLACK);
  tft_font_write_bg(&tft, 268, 222, "D", &inconsolata_lgc_bold_14, TFT_COLOR_WHITE, TFT_COLOR_BLACK);
  tft_font_write_bg(&tft, 264, 232, "B", &noto_sans_mono_bold_arrows_24, TFT_COLOR_WHITE, TFT_COLOR_BLACK);

  // origin
  tft_font_write_bg(&tft, 245, 265, ".", &noto_sans_mono_bold_26, TFT_COLOR_YELLOW, TFT_COLOR_BLACK);
...
static void els_concave_ext_r_display_header(void) {
  tft_rgb_t color = (els_concave_ext_r.locked ? TFT_COLOR_RED : TFT_COLOR_CERULEAN);
  tft_filled_rectangle(&tft, 0, 0, 480, 50, color);
  tft_font_write_bg(&tft, 8, 0, "CONCAVE EXTERNAL - R", &noto_sans_mono_bold_26, TFT_COLOR_WHITE, color);
  els_concave_ext_r_display_encoder_pips();
}
...
static void els_concave_ext_r_display_encoder_pips(void) {
  ...
  for (size_t n = 0, spacing = 0; n < 3; n++, spacing++)
    tft_filled_rectangle(&tft,
      ...
      15 + n * 10, 10,
      (n < pips ? TFT_COLOR_WHITE : els_concave_ext_r.locked ? TFT_COLOR_LITEGRAY : TFT_COLOR_BGCOLOR1));
}
