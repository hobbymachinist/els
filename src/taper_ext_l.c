...
static void els_taper_ext_l_display_axes(void) {
  ...
  if (els_taper_ext_l.show_dro) {
    ...
    els_sprint_double33(text, sizeof(text), atan(els_taper_ext_l.depth / els_taper_ext_l.length) * 180 / M_PI, NULL);
...
static void els_taper_ext_l_display_header(void) {
  if (els_taper_ext_l.locked) {
    tft_filled_rectangle(&tft, 0, 0, 480, 50, ILI9481_RED);
    tft_font_write_bg(&tft, 8, 0, "TAPER EXTERNAL - L", &noto_sans_mono_bold_26, ILI9481_WHITE, ILI9481_RED);
    ...
    tft_font_write_bg(&tft, 8, 0, "TAPER EXTERNAL - L", &noto_sans_mono_bold_26, ILI9481_WHITE, ILI9481_CERULEAN);
...
static void els_taper_ext_l_display_refresh(void) {
  ...
  if (els_taper_ext_l.op_state != els_taper_ext_l.prev_op_state) {
    ...
    if (els_taper_ext_l.op_state == ELS_TAPER_EXT_OP_TURNING && els_taper_ext_l.spring_pass_count > 0) {
...
static void els_taper_ext_l_keypad_process(void) {
  switch(els_keypad_read()) {
    ...
    case ELS_KEY_REV_FEED:
      if (els_taper_ext_l.state & (ELS_TAPER_EXT_IDLE | ELS_TAPER_EXT_PAUSED)) {
        els_taper_ext_l.only_spring_pass = !els_taper_ext_l.only_spring_pass;
        els_taper_ext_l_display_setting();
      }
      break;
    case ELS_KEY_SET_ZX:
  ...
  switch (els_taper_ext_l.op_state) {
    ...
    case ELS_TAPER_EXT_OP_MOVEZL:
      ...
      if (fabs(els_stepper->zpos) > TURN_PRECISION)
        els_stepper_move_z(-els_taper_ext_l.length - els_stepper->zpos, ELS_Z_SLO_MM_S);
      else
    ...
    case ELS_TAPER_EXT_OP_START:
      ...
      els_taper_ext_l.spring_pass_count = 0;

      if (els_taper_ext_l.only_spring_pass)
        els_stepper_move_x(-els_taper_ext_l.depth - els_stepper->xpos + 0.1, ELS_X_SLO_MM_S);
      break;
    case ELS_TAPER_EXT_OP_FEED:
      ...
      if (remaining <= TURN_PRECISION) {
        ...
        if (remaining >= (2 * els_taper_ext_l.depth_of_cut_um) / 1000.0)
          ...
          xd = remaining;
          els_taper_ext_l.spring_pass_count++;
          els_delay_reset();
        }
    ...
    case ELS_TAPER_EXT_OP_PLAN:
      ...
      els_stepper_move_xz(
        ...
        (els_taper_ext_l.spring_pass_count > 0 ? els_taper_ext_l.feed_mm_s / 2 : els_taper_ext_l.feed_mm_s)
      );
      break;
    case ELS_TAPER_EXT_OP_TURNING:
      ...
      if (fabs(els_taper_ext_l.xcurr + els_taper_ext_l.depth) > TURN_PRECISION) {
        els_stepper_move_z(-els_taper_ext_l.length - els_stepper->zpos, ELS_Z_SLO_MM_S);
    ...
    case ELS_TAPER_EXT_OP_RESETZ:
      ...
      els_stepper_move_x(els_taper_ext_l.xcurr - els_stepper->xpos, ELS_X_SLO_MM_S);
  ...
  switch(els_keypad_read()) {
    ...
    default:
      ...
      if (els_taper_ext_l.encoder_pos != encoder_curr) {
        int32_t delta = (encoder_curr - els_taper_ext_l.encoder_pos) * 100;
...
static void els_taper_ext_l_set_depth_of_cut(void) {
  ...
  switch(els_keypad_read()) {
    ...
    default:
      encoder_curr = els_encoder_read();
      if (els_taper_ext_l.encoder_pos != encoder_curr) {
        int32_t delta = (encoder_curr - els_taper_ext_l.encoder_pos) * 50;
...
void els_taper_ext_l_set_length(void) {
  ...
  switch(els_keypad_read()) {
    ...
    default:
      encoder_curr = els_encoder_read();
      if (els_taper_ext_l.encoder_pos != encoder_curr) {
        double delta = (encoder_curr - els_taper_ext_l.encoder_pos) * 0.1;
...
void els_taper_ext_l_set_depth(void) {
  ...
  switch(els_keypad_read()) {
    ...
    default:
      encoder_curr = els_encoder_read();
      if (els_taper_ext_l.encoder_pos != encoder_curr) {
        double delta = (encoder_curr - els_taper_ext_l.encoder_pos) * 0.01;
...
static void els_taper_ext_l_zjog(void) {
  ...
  if (els_taper_ext_l.encoder_pos != encoder_curr) {
    ...
    delta = (encoder_curr - els_taper_ext_l.encoder_pos);
    els_taper_ext_l.encoder_pos = encoder_curr;
