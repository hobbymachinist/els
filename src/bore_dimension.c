...
static void els_bore_dimension_display_diagram(void) {
  ...
  tft_filled_rectangle(&tft, 156, 240, 100, 40, TFT_COLOR_ORANGE);
  ...
  tft_font_write_bg(&tft, 260, 204, "D", &noto_sans_mono_bold_arrows_24, TFT_COLOR_WHITE, TFT_COLOR_BLACK);
  tft_font_write_bg(&tft, 260, 246, "2xR", &inconsolata_lgc_bold_14, TFT_COLOR_WHITE, TFT_COLOR_BLACK);
  tft_font_write_bg(&tft, 260, 264, "B", &noto_sans_mono_bold_arrows_24, TFT_COLOR_WHITE, TFT_COLOR_BLACK);
...
static void els_bore_dimension_display_header(void) {
  tft_rgb_t color = (els_bore_dimension.locked ? TFT_COLOR_RED : TFT_COLOR_DIANNE);

  tft_filled_rectangle(&tft, 0, 0, 480, 50, color);
  tft_font_write_bg(&tft, 8, 0, "BORING - POCKET", &noto_sans_mono_bold_26, TFT_COLOR_WHITE, color);
  els_bore_dimension_display_encoder_pips();
}
...
static void els_bore_dimension_display_encoder_pips(void) {
  ...
  for (size_t n = 0, spacing = 0; n < 3; n++, spacing++)
    tft_filled_rectangle(&tft,
      ...
      15 + n * 10, 10,
      (n < pips ? TFT_COLOR_WHITE : els_bore_dimension.locked ? TFT_COLOR_LITEGRAY : TFT_COLOR_BGCOLOR1));
}
